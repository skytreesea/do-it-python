...
info = [['',	'물가상승률','인건비상승률',	'수입증가율',	'인건비상승률'	],['',percent(price_index),percent(salary_increase),percent(income_increase),percent(salary_increase_new_company),str(start_year)]]
print('----주요가정-------')
k=1
while k <= 5:
    print(data_basic[0][-k]+': '+percent(float(data_basic[number][-k])))
    k += 1